defmodule SlackTest do
  ...
  test "on_connect returns state by default" do
    assert Bot.handle_connect(%Slack.State{}, 1) == {:ok, 1}
  end

  test "handle_message returns state by default" do
    assert Bot.handle_message(nil, %Slack.State{}, 1) == {:ok, 1}
  end
  ...
  test "init formats rtm results properly" do
    ...
    {:ok, %{slack: slack = %Slack.State{}, state: state}} = Bot.init(%{rtm: rtm, state: 1, client: FakeWebsocketState, token: "ABC"}, nil)
