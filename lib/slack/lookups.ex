defmodule Slack.Lookups do
  ...
  """
  def lookup_user_id("@" <> user_name, slack = %Slack{}) do
    slack.users
    |> Map.values
  ...
  """
  def lookup_direct_message_id(user = "@" <> _user_name, slack = %Slack{}) do
    user
    |> lookup_user_id(slack)
    |> lookup_direct_message_id(slack)
  end
  def lookup_direct_message_id(user_id, slack = %Slack{}) do
    slack.ims
    |> Map.values
  ...
  (`"C…"`).
  """
  def lookup_channel_id("#" <> channel_name, slack = %Slack{}) do
    slack.channels
    |> Map.values
  ...
  """
  def lookup_user_name(direct_message_id = "D" <> _id, slack = %Slack{}) do
    lookup_user_name(slack.ims[direct_message_id].user, slack)
  end
  def lookup_user_name(user_id = "U" <> _id, slack = %Slack{}) do
  ...
  """
  def lookup_channel_name(channel_id = "C" <> _id, slack = %Slack{}) do
