defmodule Slack.Lookups do
  ...
  """
  def lookup_user_id("@" <> user_name, slack = %State{}) do
    slack.users
    |> Map.values
  ...
  """
  def lookup_direct_message_id(user = "@" <> _user_name, slack = %State{}) do
    user
    |> lookup_user_id(slack)
    |> lookup_direct_message_id(slack)
  end
  def lookup_direct_message_id(user_id, slack = %State{}) do
    slack.ims
    |> Map.values
  ...
  (`"C…"`).
  """
  def lookup_channel_id("#" <> channel_name, slack = %State{}) do
    slack.channels
    |> Map.values
  ...
  """
  def lookup_user_name(direct_message_id = "D" <> _id, slack = %State{}) do
    lookup_user_name(slack.ims[direct_message_id].user, slack)
  end
  def lookup_user_name(user_id = "U" <> _id, slack = %State{}) do
  ...
  """
  def lookup_channel_name(channel_id = "C" <> _id, slack = %State{}) do
